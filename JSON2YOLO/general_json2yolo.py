...
def convert_coco_json(json_dir="../tmp_dataset/for_segment/coco_type/", saveTXT="../tmp_dataset/for_segment/yolo_data/", use_segments=False, cls91to80=False):
    """Converts COCO JSON format to YOLO label format, with options for segments and class mapping."""
    save_dir = make_dirs(saveTXT)  # output directory
...
if __name__ == "__main__":
    ...
    if source == "COCO":
        convert_coco_json(
            "./tmp_dataset_seg/coco_type/",  # directory with *.json
            use_segments=True, # use segments if available
            cls91to80=True,  # convert COCO 91 classes to 80 classes
        )
